...
public class EntityHitboxDebugRendererMixin {
    ...
    private VisibilityConfigurable hitboxplus$drawPrimaryHitbox(
        ...
        // --- CUSTOM SPEAR REACH & GREEN HITBOX LOGIC ---
        MinecraftClient client = MinecraftClient.getInstance();
        int colorToUse = style.opaqueArgb(); // Default mod color

        if (client.player != null && entity != client.player) {
            // Check if holding a spear
            boolean holdingSpear = !client.player.getMainHandStack().isEmpty() &&
                    client.player.getMainHandStack().getItem().toString().toLowerCase().contains("spear");

            double maxReach = holdingSpear ? 4.5 : 3.0; // 4.5 for Spear, 3.0 for normal weapons

            // Check if entity is inside reach distance
            if (client.player.squaredDistanceTo(entity) <= (maxReach * maxReach)) {
        ...
        // Pass updated color to patterned renderer
